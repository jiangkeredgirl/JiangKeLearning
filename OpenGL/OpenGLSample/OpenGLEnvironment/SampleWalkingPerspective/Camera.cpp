...
void Camera::setViewByMouse(int x, int y)
{
    ...
    /**< 得到鼠标移动方向 */
    angleY = (float)((middleX - mousePos.x)) / 30000.0f;
    angleZ = (float)((middleY - mousePos.y)) / 30000.0f;
    ...
    /** 如果上下旋转弧度大于1.0,我们截取到1.0并旋转 */
    if (currentRotX > max_radian)
    {
        currentRotX = max_radian;

        /** 根据保存的角度旋转方向 */
        if (lastRotX != max_radian)
        {
            /** 通过叉积找到与旋转方向垂直的向量 */
            Vector3 vAxis = m_View - m_Position;
            vAxis = vAxis.crossProduct(m_UpVector);
            ...
            ///旋转
            rotateView(max_radian - lastRotX, vAxis.x, vAxis.y, vAxis.z);
        }
    }
    /** 如果旋转弧度小于-1.0,则也截取到-1.0并旋转 */
    else if (currentRotX < -max_radian)
    {
        currentRotX = -max_radian;
...
/** 用键盘旋转摄像机 */
void Camera::setViewByKey(int x, int y)
{
    printf("setViewByKey(%d, %d)\n", x, y);
    //POINT mousePos;                                   /**< 保存当前鼠标位置 */
    //int middleX = GetSystemMetrics(SM_CXSCREEN) >> 1; /**< 得到屏幕宽度的一半 */
    //int middleY = GetSystemMetrics(SM_CYSCREEN) >> 1; /**< 得到屏幕高度的一半 */
    float angleY = 0.0f;                              /**< 摄像机左右旋转角度 */
    ...
    /**< 得到鼠标移动方向 */
    angleY = (float)((-x)) / 30000.0f;
    angleZ = (float)((-y)) / 30000.0f;
...
/** 上下摄像机移动 */
void Camera::pitchCamera(float speed)
{
    Vector3 pitch;
    Vector3 cross = m_View - m_Position;
    cross = cross.crossProduct(m_UpVector);
    ...
    // Normalize the strafe vector
    pitch = cross.normalize();
    pitch.y = 0.1;

    m_Position.y += pitch.y * speed;

    // Add the strafe vector to our view
    m_View.y += pitch.y * speed;
